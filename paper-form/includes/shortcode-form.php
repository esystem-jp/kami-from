...
function paper_form_shortcode($atts) {
    global $wpdb;
    $p = $wpdb->prefix;
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['paper_form_action']) && $_POST['paper_form_action'] === 'save') {
        if (!isset($_POST['paper_form_nonce']) || !wp_verify_nonce($_POST['paper_form_nonce'], 'paper_form_save_'.$template_id)) {
            $msg = '<div class="paper-form-error">送信の整合性確認に失敗しました（nonce）。</div>';
        } else {
            $hp = isset($_POST['pf_hp']) ? trim((string)$_POST['pf_hp']) : '';
            if ($hp !== '') {
                $msg = '<div class="paper-form-error">送信が不正と判定されました。</div>';
            } else {
                $record_date = null;
                $user_id = 0;
$vals = [];
                ...
                $res = paper_form_save_record($template_id, $record_date, $user_id, $vals);
                if ($res['ok']) {
                    $saved = $res;
                    $msg = '<div class="paper-form-ok">登録しました。番号：' . esc_html((string)$res['record_no']) . '</div>';
                } else {
                    $msg = '<div class="paper-form-error">登録に失敗しました：' . esc_html((string)$res['error']) . '</div>';
                }
            }
        }
    }
    ...
    $out  = '<style>
.paper-form-wrap{max-width:1100px;margin:0 auto;}
.paper-stage{position:relative;width:100%;border:1px solid #ddd;}
.paper-stage img{width:100%;height:auto;display:block;}
.paper-field{position:absolute;box-sizing:border-box;}
...
.paper-topbar{display:flex;gap:10px;flex-wrap:wrap;margin:10px 0;}
.paper-topbar .ctl{display:flex;gap:6px;align-items:center;}
.paper-form-ok{padding:8px 10px;background:#e7ffe7;border:1px solid #6ac46a;margin:8px 0;}
.paper-form-error{padding:8px 10px;background:#ffe7e7;border:1px solid #d26a6a;margin:8px 0;}
.paper-submit{margin:12px 0;}
.paper-submit button{font-size:18px;padding:10px 16px;}
</style>';

    $out .= '<div class="paper-form-wrap">';
    $out .= $msg;

    $out .= '<form method="post">';
    $out .= '<input type="hidden" name="paper_form_action" value="save">';
    $out .= wp_nonce_field('paper_form_save_'.$template_id, 'paper_form_nonce', true, false);

    $out .= '<div style="display:none;"><label>leave blank</label><input type="text" name="pf_hp" value=""></div>';

    $out .= '<div class="paper-topbar">';
    $out .= '';

    $out .= '<div class="ctl"><span>作業者</span></div>';
    $out .= '</div>';

    $out .= '<div class="paper-stage">';
...
$required = ((int)$f['required'] === 1);
        ...
        $out .= '<div class="paper-field" style="' . $style . '">';

        if ($type === 'master_item') {
            // 品番マスターのプルダウン
            $items = $wpdb->get_results("SELECT id, item_code, item_name FROM {$p}paper_items WHERE is_active=1 ORDER BY item_code ASC, item_name ASC", ARRAY_A);
...
if ($type === 'master_select') {
    ...
    if ($master_id > 0) {
        $opts = $wpdb->get_results($wpdb->prepare(
            "SELECT id, value_code, value_name FROM {$p}paper_master_values WHERE master_id=%d AND is_active=1 ORDER BY sort_order ASC, value_code ASC, value_name ASC",
            $master_id
        ), ARRAY_A);
    }
    $out .= '<select name="' . esc_attr($name) . '"' . ($required ? ' required' : '') . '>';
...
if ($type === 'textarea') {
    ...
    $out .= '<div class="paper-submit"><button type="submit">登録</button></div>';
    $out .= '</form>';
...
add_shortcode('paper_form', 'paper_form_shortcode');
