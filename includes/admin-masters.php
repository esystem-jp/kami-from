...
function kami_form_admin_masters_page() {
    ...
    if (($view === 'list') && (($_GET['action'] ?? '') === 'delete_master')) {
        ...
        if ($did > 0 && check_admin_referer('pf_delete_master_'.$did)) {
            $wpdb->delete("{$p}kami_master_values", ['master_id'=>$did], ['%d']);
            $wpdb->delete("{$p}kami_masters", ['id'=>$did], ['%d']);
    ...
    if (($view === 'values') && (($_GET['action'] ?? '') === 'delete_value')) {
        ...
        if ($vid > 0 && $mid > 0 && check_admin_referer('pf_delete_value_'.$mid.'_'.$vid)) {
            $wpdb->delete("{$p}kami_master_values", ['id'=>$vid, 'master_id'=>$mid], ['%d','%d']);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['pf_action'] ?? '') === 'save_value')) {
        ...
        if ($master_id <= 0) {
            ...
            if ($id > 0) {
                $wpdb->update("{$p}kami_master_values", [
    ...
    if ($view === 'values') {
        ...
        $master = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$p}kami_masters WHERE id=%d", $master_id), ARRAY_A);
        if (!$master) wp_die('マスターが見つかりません');

        echo '<p><a class="button" href="'.esc_url(admin_url('admin.php?page=kami-form-masters')).'">← マスター一覧へ</a></p>';
        echo '<h2>値一覧：'.esc_html($master['master_name']).'（'.esc_html($master['master_key']).'）</h2>';

        $edit_value_id = (int)($_GET['edit_value_id'] ?? 0);
        $edit_value = null;
        if ($edit_value_id > 0) {
            $edit_value = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$p}kami_master_values WHERE id=%d AND master_id=%d", $edit_value_id, $master_id), ARRAY_A);
        }
        ...
        $values = $wpdb->get_results($wpdb->prepare(
            "SELECT id, value_code, value_name, is_active FROM {$p}kami_master_values WHERE master_id=%d ORDER BY value_code ASC, id ASC",
            $master_id
        ), ARRAY_A);
        ...
        if (empty($values)) {
            ...
            foreach ($values as $v) {
                $del_url = wp_nonce_url(
                    admin_url('admin.php?page=kami-form-masters&view=values&master_id='.$master_id.'&action=delete_value&id='.$v['id']),
                    'pf_delete_value_'.$master_id.'_'.$v['id']
                );
                $edit_url = admin_url('admin.php?page=kami-form-masters&view=values&master_id='.$master_id.'&edit_value_id='.$v['id']);
                echo '<tr>';
    ...
    // --- Master list view ---
    $masters = $wpdb->get_results("SELECT id, master_key, master_name, is_active FROM {$p}kami_masters ORDER BY id DESC", ARRAY_A);
    $edit_master_id = (int)($_GET['edit_master_id'] ?? 0);
    $edit_master = null;
    if ($edit_master_id > 0) {
        $edit_master = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$p}kami_masters WHERE id=%d", $edit_master_id), ARRAY_A);
    }
    ...
    if (empty($masters)) {
        ...
        foreach ($masters as $m) {
            $values_url = admin_url('admin.php?page=kami-form-masters&view=values&master_id='.$m['id']);
            $edit_url   = admin_url('admin.php?page=kami-form-masters&edit_master_id='.$m['id']);
            $del_url    = wp_nonce_url(
                admin_url('admin.php?page=kami-form-masters&action=delete_master&id='.$m['id']),
