...
function kami_form_csv_escape($v) {
    $v = (string) $v;
    $v = str_replace("\r", "", $v);
    $v = str_replace("\n", "\n", $v);
    if (preg_match('/[",\n]/', $v)) {
...
add_action('admin_post_kami_form_export_csv', 'kami_form_export_csv');

/**
 * master_select の保存値（JSON / "id|code" / id）を「code name」に変換
 */
if (!function_exists('kami_form_master_display')) {
function kami_form_master_display($raw, $master_id) {
    ...
    if (!array_key_exists($ck, $cache)) {
        $row = $wpdb->get_row($wpdb->prepare("SELECT value_code, value_name FROM {$p}kami_master_values WHERE id=%d AND master_id=%d", $id, (int)$master_id), ARRAY_A);
        $cache[$ck] = $row ?: null;
    }
    $row = $cache[$ck];
    if (!$row) return $raw;
...
function kami_form_export_csv() {
    if ( ! current_user_can('edit_posts') ) {
        wp_die('権限がありません');
    }
    check_admin_referer('kami_form_export_csv');
    ...
    $sqlf = "SELECT id, template_id, label, type, master_id FROM {$p}kami_fields $field_where ORDER BY template_id ASC, sort_order ASC, id ASC";
    ...
    $sql = "SELECT r.id, r.template_id, r.record_no, r.user_id, r.status, r.created_at, t.name AS template_name, u.user_name
            FROM {$p}kami_records r
            LEFT JOIN {$p}kami_templates t ON t.id=r.template_id
            LEFT JOIN {$p}kami_users u ON u.id=r.user_id
            $where
            ORDER BY r.id DESC";
...
// values
    ...
    if (!empty($record_ids)) {
        ...
        $sqlv = "SELECT record_id, field_id, value_long FROM {$p}kami_record_values WHERE record_id IN ($in)";
...
if (!empty($need_ids)) {
    ...
    $sqlMv = "SELECT id, value_code, value_name FROM {$p}kami_master_values WHERE id IN ($in)";
    ...
    if (!empty($need_item_ids)) {
        ...
        $in = implode(',', array_fill(0, count($ids), '%d'));
        $sqlIt = "SELECT id, item_code, item_name FROM {$p}kami_items WHERE id IN ($in)";
    ...
    if (!empty($need_user_ids)) {
        ...
        $in = implode(',', array_fill(0, count($ids), '%d'));
        $sqlU = "SELECT id, user_code, user_name FROM {$p}kami_users WHERE id IN ($in)";
    ...
    $filename = 'kami_form_records_' . gmdate('Ymd') . '.csv';
    ...
    echo implode(',', array_map('kami_form_csv_escape', $header)) . "\r\n";
foreach ($rows as $r) {
        $rid = (int)$r['id'];
        $line = [
            $rid,
...
if ($is_master) {
    ...
    $line[] = (string)$val;
        ...
        echo implode(',', array_map('kami_form_csv_escape', $line)) . "\r\n";
}
