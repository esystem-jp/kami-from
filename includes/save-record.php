...
function kami_form_parse_number($s) {
    $s = trim((string)$s);
    if ($s === '') return null;
    $s = str_replace([',',' '], '', $s);
    return is_numeric($s) ? $s : null;
}
function kami_form_parse_date($s) {
    $s = trim((string)$s);
    if ($s === '') return null;
...
function kami_form_parse_time($s) {
    $s = trim((string)$s);
    if ($s === '') return null;
...
function kami_form_save_record($template_id, $user_id = 0, $arg3 = null, $arg4 = null) {
    global $wpdb;
    $p = $wpdb->prefix;
...
if (is_array($arg3) && $arg4 === null) {
    ...
    $tpl = $wpdb->get_row($wpdb->prepare(
        "SELECT id, is_active FROM {$p}kami_templates WHERE id=%d",
        $template_id
    ), ARRAY_A);
    ...
    if ($user_id > 0) {
        $u = $wpdb->get_row($wpdb->prepare(
            "SELECT mv.id, mv.is_active
             FROM {$p}kami_masters m
             JOIN {$p}kami_master_values mv ON mv.master_id=m.id
    ...
    foreach ($field_map as $fid => $f) {
        ...
        $v_long = '';
        if ($type === 'number') {
            $num = kami_form_parse_number($raw);
    ...
    try {
        ...
        $wpdb->query($wpdb->prepare(
            "INSERT IGNORE INTO {$p}kami_sequences (template_id, record_date, last_no)
             VALUES (%d, NULL, 0)",
            $template_id
        ));

        $seq = $wpdb->get_row($wpdb->prepare(
            "SELECT last_no FROM {$p}kami_sequences WHERE template_id=%d AND record_date IS NULL FOR UPDATE",
            $template_id
        ), ARRAY_A);
        ...
        $upd = $wpdb->query($wpdb->prepare(
            "UPDATE {$p}kami_sequences SET last_no=%d WHERE template_id=%d AND record_date IS NULL",
            $record_no, $template_id
        ));
        if ($upd === false) throw new Exception('seq_update_failed');

        $ok = $wpdb->query($wpdb->prepare(
            "INSERT INTO {$p}kami_records (template_id, record_date, record_no, user_id, status)
             VALUES (%d, NULL, %d, %d, %s)",
        ...
        foreach ($normalized as $fid => $v) {
            $wpdb->query($wpdb->prepare(
                "DELETE FROM {$p}kami_record_values WHERE record_id=%d AND field_id=%d",
                $record_id, (int)$fid
            ));
            if ($wpdb->last_error) throw new Exception('value_delete_failed');

            $okv = $wpdb->insert(
                "{$p}kami_record_values",
                [
                    'record_id' => $record_id,
                    'field_id' => (int)$fid,
