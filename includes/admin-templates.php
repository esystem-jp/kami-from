...
function kami_form_admin_templates_page() {
    ...
    if (isset($_GET['pf_delete']) && isset($_GET['_wpnonce'])) {
        ...
        if ($del_id > 0 && wp_verify_nonce($_GET['_wpnonce'], 'pf_delete_template_'.$del_id)) {
            $wpdb->delete("{$p}kami_templates", ['id'=>$del_id], ['%d']);
    ...
    $editing = null;
    if ($edit_id > 0) {
        $editing = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$p}kami_templates WHERE id=%d", $edit_id), ARRAY_A);
    }

    $templates = $wpdb->get_results("SELECT * FROM {$p}kami_templates ORDER BY id DESC", ARRAY_A);
    ...
    foreach ($templates as $t) {
        ...
        $del_url = wp_nonce_url(admin_url('admin.php?page=kami-forms&pf_delete='.$id), 'pf_delete_template_'.$id);
        $edit_url = admin_url('admin.php?page=kami-forms&pf_edit='.$id);
        ...
        echo '<tr>
            ...
            <td>
              ...
              <a class="button" href="'.esc_url(admin_url('admin.php?page=kami-form-fields&template_id='.$id)).'">項目設定</a>
            </td>
        </tr>';
    }
    echo '</tbody></table>';

    echo '<p class="pf-help">現場入力ページにはショートコード <code>[kami_form template="ID"]</code> を貼り付けてください。</p>';
    echo '</div>';
}

function kami_form_admin_fields_page() {
    if ( ! current_user_can('edit_posts') ) return;
    global $wpdb; $p = $wpdb->prefix;
    ...
    $tpl = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$p}kami_templates WHERE id=%d", $template_id), ARRAY_A);
...
$master_id = (int)($_POST['master_id'] ?? 0);
        ...
        if ($label === '') {
            ...
            $count = (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$p}kami_fields WHERE template_id=%d", $template_id));
            $field_key = sprintf('t%d_f%03d', $template_id, $count + 1);

            $wpdb->insert("{$p}kami_fields", [
...
$master_id = (int)($_POST['master_id'] ?? 0);
    ...
    if (isset($_GET['pf_del_field']) && isset($_GET['_wpnonce'])) {
        ...
        if ($fid>0 && wp_verify_nonce($_GET['_wpnonce'], 'pf_del_field_'.$fid)) {
            $wpdb->delete("{$p}kami_fields", ['id'=>$fid, 'template_id'=>$template_id], ['%d','%d']);
            echo '<div class="notice notice-success"><p>削除しました。</p></div>';
        }
    }
    ...
    if (isset($_GET['pf_copy_field']) && isset($_GET['_wpnonce'])) {
        ...
        if ($fid > 0 && wp_verify_nonce($_GET['_wpnonce'], 'pf_copy_field_'.$fid)) {
            $src = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$p}kami_fields WHERE id=%d AND template_id=%d", $fid, $template_id), ARRAY_A);
            if ($src) {
                $count = (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$p}kami_fields WHERE template_id=%d", $template_id));
                $field_key = sprintf('t%d_f%03d', $template_id, $count + 1);
                ...
                $ok = $wpdb->insert("{$p}kami_fields", $new);
                if ($ok) {
...
$masters_all = $wpdb->get_results("SELECT id, master_name FROM {$p}kami_masters WHERE is_active=1 ORDER BY sort_order ASC, id ASC", ARRAY_A);

    $fields = $wpdb->get_results($wpdb->prepare(
        "SELECT * FROM {$p}kami_fields WHERE template_id=%d AND type<>'label' ORDER BY sort_order ASC, id ASC",
        $template_id
    ), ARRAY_A);
    ...
    echo '<div class="pf-row"><label>タイプ</label><select name="type">'.kami_form_admin_type_options('text', 0).'</select>
          <label><input type="checkbox" name="required" value="1"> 必須</label>
          </div>';
    
echo '<div class="pf-row"><label>対象マスター</label>';
$masters = $wpdb->get_results("SELECT id, master_name FROM {$p}kami_masters WHERE is_active=1 ORDER BY sort_order ASC, id ASC", ARRAY_A);
...
echo '</select><span class="pf-help" style="margin-left:8px;">対象マスターを選ぶと、この項目は自動的にプルダウンになります</span></div>';
    ...
    foreach ($fields as $f) {
        $fid = (int)$f['id'];
        $del = wp_nonce_url(admin_url('admin.php?page=kami-form-fields&template_id='.$template_id.'&pf_del_field='.$fid), 'pf_del_field_'.$fid);
        $copy = wp_nonce_url(admin_url('admin.php?page=kami-form-fields&template_id='.$template_id.'&pf_copy_field='.$fid), 'pf_copy_field_'.$fid);
        ...
        echo '<input type="text" name="label" value="'.esc_attr($f['label']).'">
            ...
            <td><select name="type">'.kami_form_admin_type_options($f['type'], (int)($f['master_id'] ?? 0)).'</select></td>
            <td>'.kami_form_admin_master_select((int)($f['master_id'] ?? 0), $masters_all).'</td>
...
function kami_form_admin_master_select($current_master_id, $masters) {
...
function kami_form_admin_type_options($current, $master_id_unused = 0) {
    // 基本タイプ + ラベル（表示のみ）
    ...
    global $wpdb;
    $p = $wpdb->prefix;
    $masters = $wpdb->get_results("SELECT id, master_key, master_label FROM {$p}kami_masters WHERE is_active=1 ORDER BY id ASC", ARRAY_A);
    if (is_array($masters)) {
