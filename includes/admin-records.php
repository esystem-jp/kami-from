...
if (!function_exists('kami_form_master_decode')) {
    function kami_form_master_decode($raw) {
        $raw = (string)$raw;
        $idv = 0; $code = '';
        if ($raw === '') return [0,''];
...
if (!function_exists('kami_form_admin_records_page')) {
function kami_form_admin_records_page() {
    ...
    // テンプレート一覧
    $templates = $wpdb->get_results("SELECT id, name FROM {$p}kami_templates ORDER BY id DESC", ARRAY_A);
...
$params = [];
    ...
    // 件数
    $sql_count = "SELECT COUNT(*) FROM {$p}kami_records r {$where_sql}";
$total = (int)($params ? $wpdb->get_var($wpdb->prepare($sql_count, $params)) : $wpdb->get_var($sql_count));
    ...
    if (!empty($record_ids)) {
        ...
        $vals = $wpdb->get_results($wpdb->prepare(
            "SELECT record_id, field_id, value_long FROM {$p}kami_record_values WHERE record_id IN ($in)",
            $record_ids
        ), ARRAY_A);
        foreach ($vals as $v) {
            $rid = (int)$v['record_id'];
    ...
    if (!empty($master_ids)) {
        ...
        $all = $wpdb->get_results($wpdb->prepare(
            "SELECT id, master_id, value_code, value_name FROM {$p}kami_master_values WHERE master_id IN ($inm)",
            $mids
        ), ARRAY_A);
        foreach ($all as $mv) {
...
$export_url = wp_nonce_url(
    ...
    echo '<input type="hidden" name="page" value="kami-records" />';
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $r) {
            ...
            foreach ($fields as $f) {
                ...
                if ($is_master) {
                    [$idv, $code] = kami_form_master_decode($raw);
                    $name = '';
    ...
    if ($total_pages > 1) {
        $base_url = admin_url('admin.php?page=kami-records'
            . '&template_id='.(int)$template_id
            . '&date_from='.urlencode((string)$date_from)
            . '&date_to='.urlencode((string)$date_to)
        );
